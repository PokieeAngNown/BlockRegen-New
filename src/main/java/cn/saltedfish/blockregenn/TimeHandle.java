...
public class TimeHandle {
    ...
    public static final long DeepBlockRegenTime = BlockRegenN.getPlugin().getConfig().getLong("DeepBlockRegenTime");
    public static final long MiddleBlockRegenTime = BlockRegenN.getPlugin().getConfig().getLong("MiddleBlockRegenTime");
    private static final int DetectTime = BlockRegenN.getPlugin().getConfig().getInt("DetectTime");
    ...
    public static void runTimeTask(){
        ...
        new BukkitRunnable(){
            @Override
            public void run(){
                File file = new File(BlockRegenN.getPlugin().getDataFolder() + "/data/");
                if (file.isDirectory()) {
        ...
        }.runTaskTimer(BlockRegenN.getPlugin(), DetectTime * 20L, DetectTime * 20L);
    }
